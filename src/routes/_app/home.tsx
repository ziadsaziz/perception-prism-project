  ...
  useEffect(() => {
    ...
    (async () => {
      const [p, s, pat, scansData, ls] = await Promise.all([
        ...
        supabase.from("daily_checkins").select("move_completed").eq("user_id", user.id).eq("date", new Date().toISOString().slice(0, 10)).maybeSingle(),
      ]);

      setProfile(p.data);
      setScores(s.data as any);
      setPattern(pat.data);
      setScanCount(scansData.data?.length ?? 0);
      setRecentScans((scansData.data ?? []).slice(0, 3));

      const lastScan = scansData.data?.[0];
      if (lastScan?.created_at) {
        const days = Math.floor((Date.now() - new Date(lastScan.created_at).getTime()) / 86400000);
        setLastScanDaysAgo(days);
      }

      if (ls.data?.move_completed) setMoveDone(true);
      setProfileChecked(true);
      // Show tutorial for new users who haven't seen it
      if (!p.data?.tutorial_completed && p.data?.onboarding_complete) {
        // Store flag — tutorial shows after check-in is resolved
        setShowTutorial(true);
      }
      // Show setup if name is missing
      const autoGenerated =
        !p.data?.name ||
        (p.data.name.length < 20 &&
          !p.data.name.includes(" ") &&
          p.data?.main_goal === null);
      if (autoGenerated) setShowSetup(true);
    })();
  }, [user]);
      ...
      <Link to="/predictions" className="block bg-glass ring-hairline rounded-2xl p-5 border border-[#C9A84C]/20 active:scale-[0.99] transition-transform">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-[0.28em] text-[#C9A84C] mb-1">Mirror Predictions</p>
            <p className="text-sm text-foreground/90 leading-snug">What Mirror sees coming next.</p>
            <p className="mt-1.5 text-[10px] uppercase tracking-[0.24em] text-muted-foreground">See predictions →</p>
          </div>
