...
function Home() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [p, s, pat, sc, ls, rs] = await Promise.all([
        ...
        supabase.from("scans").select("created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
        supabase.from("scans").select("scan_type, ai_summary, result_json, created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(3),
      ]);
      setProfile(p.data);
      setScores(s.data as any);
      setPattern(pat.data);
      setScanCount(sc.count ?? 0);
      if (ls.data?.created_at) {
        const days = Math.floor((Date.now() - new Date(ls.data.created_at).getTime()) / 86400000);
      ...
      const { data: checkin } = await supabase
        .from("daily_checkins")
        .select("move_completed")
        .eq("user_id", user.id)
        .eq("date", today)
        .maybeSingle();
      if (checkin?.move_completed) setMoveDone(true);
